...
**Package baseline:** `v1.2-repository-refresh` (April 2026). Public cockpit UI: [https://www.ai-companion.org/](https://www.ai-companion.org/).
...
| Benchmark package | Ready: DB-verified exports under `backend/exports/` (`campaign_insights_latest.*`, `workflow_insights_bundle.*`, plus `full_campaign_analysis_bundle.*` and `campaign_agentic_debug.*`) reflect 10/10 successful seeded campaign analyzes in the current bundle |
...
- Fresh benchmark artifacts in-repo: `backend/exports/campaign_insights_latest.*`, `backend/exports/workflow_insights_bundle.*`, `backend/exports/full_campaign_analysis_bundle.*`, `backend/exports/campaign_agentic_debug.*`
...
- Submission package revision: April 2026, baseline `v1.2-repository-refresh`; production UI URL updated to `https://www.ai-companion.org/`.
